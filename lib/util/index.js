...
/**
 * Removes the row at a specified index.
 */
export function removeRow(layout, rowIndex) {
  const rows = Object.assign([], layout.rows);
 ...
 * Sorts a widget in the same column.
 */
export function sortRow(layout, initialLocation, destination) {
  const rows = Object.assign([], layout.rows);
  const row = Object.assign({}, rows[initialLocation.rowIndex]);

  rows.splice(initialLocation.rowIndex, 1);
  rows.splice(destination.rowIndex, 0, row);
