...
const columnTarget = {
  drop(props, monitor) {
    const { layout, rowIndex, columnIndex, onChangeLayout } = props;
    const item = monitor.getItem();

    if (item.columnIndex !== columnIndex || item.rowIndex !== rowIndex) {
      delete item.props.widgetData;
      Object.keys(props.widgets[item.widgetName].props).forEach((key) => {
...
class Column extends Component {
  render() {
    ...
    if (addWidgetComponent) {
      addWidgetComponentToUse = createElement(addWidgetComponent, {
        text: addWidgetComponentText,
        onClick: () => onAdd(layout, rowIndex, columnIndex),
      });
    } else {
